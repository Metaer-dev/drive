...
frappe.ui.form.on("Drive Data Import", {
  setup(frm) {
  ...
  // 					total_records,
  // 				]);
  // 			} else if(frm.doc.import_type === "Cover All Records"){
  // 				message = __("Successfully covered {0} out of {1} records.", [
  // 					successful_records,
  // 					total_records,
  // 				])
  //             }
    ...
    frm
      .call({
        ...
        args: {
          ...
          drive_data_import_name: frm.doc.name,
        },
        error_handlers: {
  ...
  render_import_log(frm) {
    frm.call({
      ...
      args: {
        drive_data_import: frm.doc.name,
      },
      callback: function (r) {
        let logs = r.message;
          ...
          .map((log) => {
            ...
            if (log.success) {
              if (frm.doc.import_type === "Insert New Records") {
                ...
                html = __("Successfully covered {0}", [
                  `<span class="underline">${frappe.utils.get_form_link(
                    frm.doc.reference_doctype,
                    log.docname,
                    true
